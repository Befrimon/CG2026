...
Renderer::~Renderer() {
    ...
    for (int i = 0; i < FRAMES; i++) {
        vkDestroyBuffer(m_dev, m_geomUb[i], nullptr);
        vkFreeMemory(m_dev, m_geomUbm[i], nullptr);
        vkDestroyBuffer(m_dev, m_globalUb[i], nullptr);
        vkFreeMemory(m_dev, m_globalUbm[i], nullptr);
    }
    ...
    vkDestroyPipeline(m_dev, m_geomPipe, nullptr);
    vkDestroyPipelineLayout(m_dev, m_geomPl, nullptr);
    vkDestroyPipeline(m_dev, m_lightPipe, nullptr);
    vkDestroyPipelineLayout(m_dev, m_lightPl, nullptr);
    vkDestroyPipeline(m_dev, m_shadowPipe, nullptr);
    vkDestroyPipelineLayout(m_dev, m_shadowPl, nullptr);

    vkDestroyRenderPass(m_dev, m_mainRp, nullptr);
...
void Renderer::initSurface() {
    if (glfwCreateWindowSurface(m_inst, m_win.handle(), nullptr, &m_surf) != VK_SUCCESS) throw std::runtime_error("");
}
...
void Renderer::initDevice() {
    ...
    if (!m_gpu) throw std::runtime_error("");
    ...
    q1.queueFamilyIndex = m_gfxFam;
    q1.queueCount = 1;
    q1.pQueuePriorities = &prio;
    qcis.push_back(q1);
    if (m_gfxFam != m_presFam) {
        VkDeviceQueueCreateInfo q2{};
        q2.sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
        q2.queueFamilyIndex = m_presFam;
        q2.queueCount = 1;
        q2.pQueuePriorities = &prio;
        qcis.push_back(q2);
    }
...
void Renderer::initSwapchain() {
    ...
    if (sc.caps.maxImageCount > 0) count = std::min(count, sc.caps.maxImageCount);
...
void Renderer::initShadowMap() {
    for (int i = 0; i < FRAMES; i++) {
        mkImg(2048, 2048, VK_FORMAT_D32_SFLOAT, VK_IMAGE_TILING_OPTIMAL,
              VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
              VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_shadowImg[i], m_shadowMem[i]);
...
void Renderer::initGBuffers() {
    ...
    VkFormat dFmt = depthFmt();
    mkImg(m_scExt.width, m_scExt.height, dFmt, VK_IMAGE_TILING_OPTIMAL,
...
void Renderer::initRenderPasses() {
    ...
    sd.format = VK_FORMAT_D32_SFLOAT;
    ...
    sd.finalLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    ...
    VkSubpassDependency sDep{};
    sDep.srcSubpass = 0;
    sDep.dstSubpass = VK_SUBPASS_EXTERNAL;
    sDep.srcStageMask = VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;
    sDep.dstStageMask = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;
    sDep.srcAccessMask = VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
    sDep.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;

    VkRenderPassCreateInfo sci{};
    sci.sType = VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO;
    sci.attachmentCount = 1;
    sci.pAttachments = &sd;
    sci.subpassCount = 1;
    sci.pSubpasses = &sSub;
    sci.dependencyCount = 1;
    sci.pDependencies = &sDep;
    ...
    for (int i = 2; i <= 4; i++) {
        ...
        atts[i].storeOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
    ...
    atts[3].format = VK_FORMAT_R16G16B16A16_SFLOAT;
    atts[4].format = VK_FORMAT_R8G8B8A8_UNORM;
    ...
    std::array<VkAttachmentReference, 3> lInputRefs = {{
    ...
    subpasses[0].colorAttachmentCount = gColorRefs.size();
    subpasses[0].pColorAttachments = gColorRefs.data();
    subpasses[0].pDepthStencilAttachment = &gDepthRef;

    subpasses[1].pipelineBindPoint = VK_PIPELINE_BIND_POINT_GRAPHICS;
    subpasses[1].inputAttachmentCount = lInputRefs.size();
    subpasses[1].pInputAttachments = lInputRefs.data();
    subpasses[1].colorAttachmentCount = 1;
    subpasses[1].pColorAttachments = &lColorRef;
    ...
    VkRenderPassCreateInfo mci{};
    mci.sType = VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO;
    mci.attachmentCount = atts.size();
    mci.pAttachments = atts.data();
    mci.subpassCount = subpasses.size();
    mci.pSubpasses = subpasses.data();
    mci.dependencyCount = deps.size();
    mci.pDependencies = deps.data();
...
void Renderer::initFramebuffers() {
    for (int i = 0; i < FRAMES; i++) {
        ...
        sfci.renderPass = m_shadowRp;
        sfci.attachmentCount = 1;
        sfci.pAttachments = &m_shadowView[i];
        sfci.width = 2048;
        sfci.height = 2048;
        sfci.layers = 1;
        vkCreateFramebuffer(m_dev, &sfci, nullptr, &m_shadowFb[i]);
    }
...
void Renderer::initDescLayouts() {
    std::array<VkDescriptorSetLayoutBinding, 3> gB{};
    gB[0].binding = 0; gB[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER; gB[0].descriptorCount = 1; gB[0].stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
    ...
    VkDescriptorSetLayoutCreateInfo gCi{};
    gCi.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    gCi.bindingCount = gB.size(); gCi.pBindings = gB.data();
    ...
    lCi.bindingCount = lB.size(); lCi.pBindings = lB.data();
...
void Renderer::initPipelines() {
    ...
    VkPushConstantRange gPr{};
    gPr.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
    gPr.size = sizeof(GeomPush);

    VkPipelineLayoutCreateInfo gPli{};
    gPli.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    gPli.setLayoutCount = 1; gPli.pSetLayouts = &m_geomDsl;
    gPli.pushConstantRangeCount = 1; gPli.pPushConstantRanges = &gPr;
    vkCreatePipelineLayout(m_dev, &gPli, nullptr, &m_geomPl);

    VkPushConstantRange sPr{};
    sPr.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
    sPr.size = sizeof(ShadowPush);
    VkPipelineLayoutCreateInfo sPli{};
    sPli.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    sPli.pushConstantRangeCount = 1; sPli.pPushConstantRanges = &sPr;
    vkCreatePipelineLayout(m_dev, &sPli, nullptr, &m_shadowPl);

    VkPipelineLayoutCreateInfo lPli{};
    lPli.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    lPli.setLayoutCount = 1; lPli.pSetLayouts = &m_lightDsl;
    vkCreatePipelineLayout(m_dev, &lPli, nullptr, &m_lightPl);

    VkPipelineShaderStageCreateInfo ss[2]{};
    ss[0].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO; ss[0].pName = "main";
    ss[1].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO; ss[1].pName = "main";
    ...
    vi.vertexAttributeDescriptionCount = ad.size(); vi.pVertexAttributeDescriptions = ad.data();
    ...
    std::array<VkPipelineColorBlendAttachmentState, 3> cbG{};
    for (int i = 0; i < 3; i++) cbG[i].colorWriteMask = 0xF;
    VkPipelineColorBlendStateCreateInfo cbGS{};
    cbGS.sType = VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO;
    cbGS.attachmentCount = cbG.size(); cbGS.pAttachments = cbG.data();

    std::vector<VkDynamicState> dynStates = {VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_SCISSOR};
    VkPipelineDynamicStateCreateInfo dyn{};
    dyn.sType = VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO;
    dyn.dynamicStateCount = dynStates.size(); dyn.pDynamicStates = dynStates.data();

    ss[0].stage = VK_SHADER_STAGE_VERTEX_BIT; ss[0].module = vG;
    ss[1].stage = VK_SHADER_STAGE_FRAGMENT_BIT; ss[1].module = fG;
    VkGraphicsPipelineCreateInfo gCi{};
    gCi.sType = VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO;
    gCi.stageCount = 2; gCi.pStages = ss;
    gCi.pVertexInputState = &vi; gCi.pInputAssemblyState = &ia; gCi.pViewportState = &vp;
    gCi.pRasterizationState = &rs; gCi.pMultisampleState = &ms; gCi.pDepthStencilState = &ds;
    gCi.pColorBlendState = &cbGS; gCi.pDynamicState = &dyn;
    ...
    VkPipelineVertexInputStateCreateInfo viE{};
    viE.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
    ds.depthTestEnable = VK_FALSE; ds.depthWriteEnable = VK_FALSE;
    VkPipelineColorBlendAttachmentState cbL{};
    cbL.colorWriteMask = 0xF;
    VkPipelineColorBlendStateCreateInfo cbLS{};
    cbLS.sType = VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO;
    cbLS.attachmentCount = 1; cbLS.pAttachments = &cbL;
    ss[0].module = vL; ss[1].module = fL;
    VkGraphicsPipelineCreateInfo lCi = gCi;
    lCi.pVertexInputState = &viE; lCi.pDepthStencilState = &ds; lCi.pColorBlendState = &cbLS;
    lCi.layout = m_lightPl; lCi.subpass = 1;
    ...
    dyn.dynamicStateCount = dynStates.size(); dyn.pDynamicStates = dynStates.data();
    ss[0].module = vS;
    VkGraphicsPipelineCreateInfo sCi = gCi;
    sCi.stageCount = 1; sCi.pStages = ss; sCi.pDepthStencilState = &ds; sCi.pColorBlendState = nullptr;
    sCi.layout = m_shadowPl; sCi.renderPass = m_shadowRp; sCi.subpass = 0;
    vkCreateGraphicsPipelines(m_dev, VK_NULL_HANDLE, 1, &sCi, nullptr, &m_shadowPipe);

    vkDestroyShaderModule(m_dev, vG, nullptr); vkDestroyShaderModule(m_dev, fG, nullptr);
    vkDestroyShaderModule(m_dev, vL, nullptr); vkDestroyShaderModule(m_dev, fL, nullptr);
...
void Renderer::initWhiteTex() {
    ...
    mkImg(1, 1, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT|VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_wImg, m_wMem);
    transitionImg(m_wImg, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    cpBufToImg(sb, m_wImg, 1, 1);
    transitionImg(m_wImg, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
void Renderer::initDescPool() {
    std::array<VkDescriptorPoolSize, 3> sz = {{
        ...
        {VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, (2 * MAX_MESHES + 1) * FRAMES},
        {VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, 3 * FRAMES}
    }};
    VkDescriptorPoolCreateInfo ci{};
    ci.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    ci.poolSizeCount = sz.size(); ci.pPoolSizes = sz.data();
    ci.maxSets = (MAX_MESHES + 1) * FRAMES;
    vkCreateDescriptorPool(m_dev, &ci, nullptr, &m_dp);
}

void Renderer::allocLightDescSets() {
    std::vector<VkDescriptorSetLayout> layouts(FRAMES, m_lightDsl);
    VkDescriptorSetAllocateInfo ai{};
    ai.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    ai.descriptorPool = m_dp;
    ai.descriptorSetCount = FRAMES;
    ai.pSetLayouts = layouts.data();
    vkAllocateDescriptorSets(m_dev, &ai, m_lightDs.data());
}
...
void Renderer::initCmdBuffers() {
    ...
    ai.commandPool = m_cp; ai.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY; ai.commandBufferCount = FRAMES;
...
void Renderer::allocMeshDescSets(GpuMesh& gm) {
    std::vector<VkDescriptorSetLayout> layouts(FRAMES, m_geomDsl);
    VkDescriptorSetAllocateInfo ai{};
    ai.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
...
void Renderer::writeMeshDescSets(GpuMesh& gm) {
    ...
    for (int i = 0; i < FRAMES; i++) {
        ...
        w[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET; w[1].dstSet = gm.ds[i]; w[1].dstBinding = 1; w[1].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER; w[1].descriptorCount = 1; w[1].pImageInfo = &iI1;
        w[2].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET; w[2].dstSet = gm.ds[i]; w[2].dstBinding = 2; w[2].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER; w[2].descriptorCount = 1; w[2].pImageInfo = &iI2;
        vkUpdateDescriptorSets(m_dev, w.size(), w.data(), 0, nullptr);
    }
}
...
void Renderer::uploadTextureToMesh(GpuMesh& gm, const std::string& path, int slot) {
    ...
    void* p; vkMapMemory(m_dev, sm, 0, sz, 0, &p); memcpy(p, px, sz); vkUnmapMemory(m_dev, sm);
    stbi_image_free(px);
    VkImage& tImg = slot == 0 ? gm.texImg : gm.tex2Img;
    VkDeviceMemory& tMem = slot == 0 ? gm.texMem : gm.tex2Mem;
    mkImg(w, h, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT|VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, tImg, tMem);
    transitionImg(tImg, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    cpBufToImg(sb, tImg, w, h);
    transitionImg(tImg, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    vkDestroyBuffer(m_dev, sb, nullptr); vkFreeMemory(m_dev, sm, nullptr);
    VkSamplerCreateInfo si{};
    si.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO; si.magFilter = VK_FILTER_LINEAR; si.minFilter = VK_FILTER_LINEAR; si.addressModeU = VK_SAMPLER_ADDRESS_MODE_REPEAT; si.addressModeV = VK_SAMPLER_ADDRESS_MODE_REPEAT; si.anisotropyEnable = VK_TRUE; si.maxAnisotropy = 16.f; si.mipmapMode = VK_SAMPLER_MIPMAP_MODE_LINEAR;
    if (slot == 0) { gm.texView = mkView(tImg, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT); vkCreateSampler(m_dev, &si, nullptr, &gm.texSampler); gm.hasTex = true; }
    else { gm.tex2View = mkView(tImg, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT); vkCreateSampler(m_dev, &si, nullptr, &gm.tex2Sampler); gm.hasTex2 = true; }
}
...
void Renderer::uploadMesh(const MeshData& mesh) {
    ...
    if (!mesh.texturePath.empty() && fs::exists(mesh.texturePath)) uploadTextureToMesh(gm, mesh.texturePath, 0);
    if (!mesh.texturePath2.empty() && fs::exists(mesh.texturePath2)) uploadTextureToMesh(gm, mesh.texturePath2, 1);
...
void Renderer::clearMeshes() {
    ...
    for (auto& gm : m_meshes) {
        ...
        if (gm.hasTex) { vkDestroySampler(m_dev, gm.texSampler, nullptr); vkDestroyImageView(m_dev, gm.texView, nullptr); vkDestroyImage(m_dev, gm.texImg, nullptr); vkFreeMemory(m_dev, gm.texMem, nullptr); }
        if (gm.hasTex2) { vkDestroySampler(m_dev, gm.tex2Sampler, nullptr); vkDestroyImageView(m_dev, gm.tex2View, nullptr); vkDestroyImage(m_dev, gm.tex2Img, nullptr); vkFreeMemory(m_dev, gm.tex2Mem, nullptr); }
    }
    m_meshes.clear();
}
...
void Renderer::renderFrame(glm::vec3 camPos, glm::mat4 view, glm::mat4 proj) {
    ...
    // Сдвиг глубины чтобы предотвратить shadow acne (само-затенение)
    vkCmdSetDepthBias(cmd, 1.25f, 0.0f, 1.75f);

    for (size_t i = 0; i < m_lights.size(); i++) {
        int quad = (int)m_lights[i].info.w;
        if (quad < 0) continue;

        float qx = (quad % 2) * 1024.f; float qy = (quad / 2) * 1024.f;
        VkViewport vp{qx, qy, 1024.f, 1024.f, 0.f, 1.f};
        vkCmdSetViewport(cmd, 0, 1, &vp);
        VkRect2D sc{{(int32_t)qx, (int32_t)qy}, {1024, 1024}};
    ...
    mClr[0].color = {{0.f, 0.f, 0.f, 1.f}};
    mClr[1].depthStencil = {1.f, 0};
    mClr[2].color = {{0.f, 0.f, 0.f, 0.f}};
    mClr[3].color = {{0.f, 0.f, 0.f, 0.f}};
    mClr[4].color = {{0.f, 0.f, 0.f, 0.f}};
    VkRenderPassBeginInfo mRpi{}; mRpi.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO; mRpi.renderPass = m_mainRp;
    mRpi.framebuffer = m_fbs[m_imgIdx]; mRpi.renderArea.extent = m_scExt;
    mRpi.clearValueCount = mClr.size(); mRpi.pClearValues = mClr.data();
    vkCmdBeginRenderPass(cmd, &mRpi, VK_SUBPASS_CONTENTS_INLINE);
    vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, m_geomPipe);
    VkViewport mvp{0, 0, (float)m_scExt.width, (float)m_scExt.height, 0, 1};
    vkCmdSetViewport(cmd, 0, 1, &mvp);
    VkRect2D msc{{0,0}, m_scExt};
    ...
    for (auto& d : m_draws) {
        ...
        GeomPush gp{d.model, gm.mat.diffuse, gm.mat.specular, gm.mat.ambient, gm.mat.shininess};
        vkCmdPushConstants(cmd, m_geomPl, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(GeomPush), &gp);
        VkDeviceSize off = 0;
        vkCmdBindVertexBuffers(cmd, 0, 1, &gm.vb, &off);
        vkCmdBindIndexBuffer(cmd, gm.ib, 0, VK_INDEX_TYPE_UINT32);
        vkCmdDrawIndexed(cmd, gm.count, 1, 0, 0, 0);
    ...
    if (r == VK_ERROR_OUT_OF_DATE_KHR || r == VK_SUBOPTIMAL_KHR || m_win.resized()) {
        m_win.clearResize(); recreateSwapchain();
    }
...
void Renderer::destroySwapchain() {
    for (int i = 0; i < FRAMES; i++) {
        vkDestroyFramebuffer(m_dev, m_shadowFb[i], nullptr);
        vkDestroyImageView(m_dev, m_shadowView[i], nullptr);
        vkDestroyImage(m_dev, m_shadowImg[i], nullptr);
        vkFreeMemory(m_dev, m_shadowMem[i], nullptr);
    }
...
void Renderer::recreateSwapchain() {
    ...
    vkDeviceWaitIdle(m_dev);
    destroySwapchain();
    initSwapchain(); initImageViews(); initShadowMap(); initGBuffers(); initFramebuffers(); updateLightDescSets();
}
...
Renderer::QFI Renderer::findQFI(VkPhysicalDevice dev) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        VkBool32 ps = false; vkGetPhysicalDeviceSurfaceSupportKHR(dev, i, m_surf, &ps);
        if (ps) qi.present = i;
        if (qi.ok()) break;
    }
    return qi;
...
Renderer::SCSupport Renderer::querySC(VkPhysicalDevice dev) {
    SCSupport s; vkGetPhysicalDeviceSurfaceCapabilitiesKHR(dev, m_surf, &s.caps);
    uint32_t n; vkGetPhysicalDeviceSurfaceFormatsKHR(dev, m_surf, &n, nullptr);
...
bool Renderer::deviceOk(VkPhysicalDevice dev) {
    ...
    std::vector<VkExtensionProperties> exts(n); vkEnumerateDeviceExtensionProperties(dev, nullptr, &n, exts.data());
...
VkFormat Renderer::depthFmt() {
    return findFmt({VK_FORMAT_D32_SFLOAT, VK_FORMAT_D32_SFLOAT_S8_UINT, VK_FORMAT_D24_UNORM_S8_UINT}, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT);
}
...
VkShaderModule Renderer::makeShader(const std::vector<char>& code) {
    ...
    VkShaderModule m; if (vkCreateShaderModule(m_dev, &ci, nullptr, &m) != VK_SUCCESS) throw std::runtime_error("");
    return m;
}
...
void Renderer::mkBuf(VkDeviceSize sz, VkBufferUsageFlags usage, VkMemoryPropertyFlags props, VkBuffer& buf, VkDeviceMemory& mem) {
    VkBufferCreateInfo ci{VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO};
    ci.size = sz; ci.usage = usage; ci.sharingMode = VK_SHARING_MODE_EXCLUSIVE; vkCreateBuffer(m_dev, &ci, nullptr, &buf);
    ...
    vkAllocateMemory(m_dev, &ai, nullptr, &mem); vkBindBufferMemory(m_dev, buf, mem, 0);
}

void Renderer::cpBuf(VkBuffer src, VkBuffer dst, VkDeviceSize sz) {
    auto cmd = beginOnce(); VkBufferCopy c{0, 0, sz}; vkCmdCopyBuffer(cmd, src, dst, 1, &c); endOnce(cmd);
}

void Renderer::mkImg(uint32_t w, uint32_t h, VkFormat fmt, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags props, VkImage& img, VkDeviceMemory& mem) {
    VkImageCreateInfo ci{VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO};
    ci.imageType = VK_IMAGE_TYPE_2D; ci.format = fmt; ci.extent = {w,h,1}; ci.mipLevels = 1; ci.arrayLayers = 1; ci.samples = VK_SAMPLE_COUNT_1_BIT;
    ci.tiling = tiling; ci.usage = usage; ci.sharingMode = VK_SHARING_MODE_EXCLUSIVE; vkCreateImage(m_dev, &ci, nullptr, &img);
    VkMemoryRequirements req; vkGetImageMemoryRequirements(m_dev, img, &req);
    VkMemoryAllocateInfo ai{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};
    ai.allocationSize = req.size; ai.memoryTypeIndex = memType(req.memoryTypeBits, props);
    vkAllocateMemory(m_dev, &ai, nullptr, &mem); vkBindImageMemory(m_dev, img, mem, 0);
}

VkImageView Renderer::mkView(VkImage img, VkFormat fmt, VkImageAspectFlags aspect) {
    VkImageViewCreateInfo ci{VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO}; ci.image = img; ci.viewType = VK_IMAGE_VIEW_TYPE_2D; ci.format = fmt;
    ci.subresourceRange = {aspect, 0, 1, 0, 1}; VkImageView v; vkCreateImageView(m_dev, &ci, nullptr, &v); return v;
}
...
void Renderer::transitionImg(VkImage img, VkFormat, VkImageLayout from, VkImageLayout to) {
    ...
    if (from == VK_IMAGE_LAYOUT_UNDEFINED) { b.srcAccessMask = 0; b.dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT; src = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT; dst = VK_PIPELINE_STAGE_TRANSFER_BIT; }
    else { b.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT; b.dstAccessMask = VK_ACCESS_SHADER_READ_BIT; src = VK_PIPELINE_STAGE_TRANSFER_BIT; dst = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT; }
    vkCmdPipelineBarrier(cmd, src, dst, 0, 0, nullptr, 0, nullptr, 1, &b); endOnce(cmd);
}

void Renderer::cpBufToImg(VkBuffer buf, VkImage img, uint32_t w, uint32_t h) {
    auto cmd = beginOnce(); VkBufferImageCopy r{}; r.imageSubresource = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1}; r.imageExtent = {w, h, 1};
    vkCmdCopyBufferToImage(cmd, buf, img, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &r); endOnce(cmd);
}

VkCommandBuffer Renderer::beginOnce() {
    VkCommandBufferAllocateInfo ai{VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO}; ai.commandPool = m_cp; ai.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY; ai.commandBufferCount = 1;
...
void Renderer::endOnce(VkCommandBuffer cmd) {
    ...
    vkQueueSubmit(m_gfxQ, 1, &si, VK_NULL_HANDLE); vkQueueWaitIdle(m_gfxQ); vkFreeCommandBuffers(m_dev, m_cp, 1, &cmd);
}
